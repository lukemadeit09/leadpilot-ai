...
def analyze_lead(
    ...
    lead, analysis, task, activity = AILeadWorkflow().run(db, current_user, payload)
...
def generate_reply(payload: ReplyRequest, current_user: User = Depends(get_current_user)) -> ReplyResponse:
